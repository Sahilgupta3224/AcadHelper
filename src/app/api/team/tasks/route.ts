...
export async function POST(request:NextRequest){
   try{
      ...
      return NextResponse.json({message:"Task added sucessfully",tasks:team.tasks,task:team.tasks[team.tasks.length-1],success:true})
   }catch(error:any){
...
export async function PUT(request:NextRequest){
    try{
        ...
        const updatedTeam = await Team.findOneAndUpdate({_id:teamId,"tasks._id":taskId},{$set:{"tasks.$.completed":completed}},{new:true})
